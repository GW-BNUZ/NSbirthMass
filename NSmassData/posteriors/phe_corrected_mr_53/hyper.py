...
data_exps=np.loadtxt(p_parent_path+'/{}.txt'.format(parent_dir_name))
data_exp=abs(data_exps[::2])
import sys
sys.path.append(p_parent_path)
import utilizes
npool=4
NN=int(len(data_exp)/5000)
nlive=400
mx=5000
#################################
data_df=list()
...
hyper_prior=eval('utilizes.hyper_prior_{}'.format(current_dir_name) )
...
hp_priors =eval('utilizes.hp_priors_{}'.format(current_dir_name) ) 
