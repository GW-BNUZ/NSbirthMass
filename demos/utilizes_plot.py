...
def hyper_prior_U(dataset,mlo,mup):
    return (( dataset >= mlo) & (dataset <= mup) ) / abs(mup-mlo)
...
def hyper_prior_sst(dataset, mu,sigma,nu,tau):
        ...
        z = (dataset - mu_0) / sigma_0
        p = np.where(dataset < mu_0,
...
def hyper_prior_gamma(dataset, k,theta):
    return (1 / (gamma(k)*theta**k)) * dataset**(k-1) *np.exp(-dataset/theta)
...
def hyper_prior_G(dataset, mu, sigma,mlo,mup):
    ...
    return ( ( dataset >= mlo) & (dataset <= mup))*((mu>mlo)&(mu<mup)) * (np.exp(- (dataset - mu)**2 / (2 * sigma**2)) /\
...
def hyper_prior_2G(dataset, mu1, sigma1,mu2,sigma2,alpha,mup,mlo):
    ...
    return ((mu2 < mup ) & (mu1 > mlo) & (mu1 < mu2)  & ( dataset >= mlo) & (dataset <= mup)) *\
...
def hyper_prior_3G_fixed_max_min(dataset, mu1, sigma1,mu2,sigma2,alpha,mu3,sigma3,beta):
    ...
    if mu1 < mu2 and mu3>mu2  and alpha+beta<=1:
        return ((alpha*(np.exp(- (dataset - mu1)**2 / (2 * sigma1**2)) /(2 * np.pi * sigma1**2)**0.5))/normalisingTerm1)\
        +((beta*(np.exp(- (dataset - mu2)**2 / (2 * sigma2**2)) /(2 * np.pi * sigma2**2)**0.5))/normalisingTerm2)\
        +(((1-alpha-beta)*(np.exp(- (dataset - mu3)**2 / (2 * sigma3**2)) /(2 * np.pi * sigma3**2)**0.5))/normalisingTerm3)
    else:
        return 0
...
def hyper_prior_turn_on_pow_fix(dataset, alpha, mmin, delta_m):
    ...
    probability_fix = turn_on_pow_fix(dataset, parameters, pow_norm_fix)
...
def hyper_prior_turn_on_pow_G(dataset,alpha, mmin, mmax, delta_m, mu_g,sigma_g,lam):
    return norm_pow_add_G(dataset,alpha, mmin, mmax, delta_m, mu_g,sigma_g,lam)
...
def hyper_prior_turn_on_pow_G_fixed_max(dataset,alpha, mmin, delta_m, mu_g,sigma_g,lam):
    mmax=2.9
    return norm_pow_add_G_fixed_max(dataset,alpha, mmin, delta_m, mu_g,sigma_g,lam)
