...
##########test different fixed maximum mass affact the evidence of top_fix_max model
#begin turn_on_pow with fixed m_max 2.3
fix_max_m_2p3=2.3
def window_fix(masses, mmin,  delta_m):
    mmaxs_fix=fix_max_m_2p3
    mmax=mmaxs_fix
...
def norm_ppow_fix(parameters):
    """normalise ppow, requires m1s, an array of m values, and dm, the spacing of
    that array"""
    m1s = np.linspace(0.9, fix_max_m_2p3, 500)
    return np.trapz(ppow_fix(m1s, parameters), m1s)
...
def hyper_prior_turn_on_pow_fix_2p3(dataset, alpha, mmin, delta_m):
    parameters = dict(alpha=alpha, mmin=mmin, delta_m=delta_m)
    pow_norm_fix = norm_ppow_fix(parameters)
    probability_fix = turn_on_pow_fix(dataset['mu'], parameters, pow_norm_fix)
    return probability_fix

hp_priors_turn_on_pow_fix_2p3 = dict(alpha=Uniform(-5, 25, 'alpha', '$\\alpha$'),
                 mmin=Uniform(0.9, 1.5, 'mmin', '$mmin$'),
                delta_m=Uniform(0.01, 1, 'delta', '$\\delta$') )
#end turn_on_pow fixed m_max 2.3

#begin turn_on_pow with fixed m_max 1.7
fix_max_m_1p7=1.7
def window_fix(masses, mmin,  delta_m):
    mmaxs_fix=fix_max_m_1p7
    mmax=mmaxs_fix
...
def norm_ppow_fix(parameters):
    """normalise ppow, requires m1s, an array of m values, and dm, the spacing of
    that array"""
    m1s = np.linspace(0.9, fix_max_m_1p7, 500)
    return np.trapz(ppow_fix(m1s, parameters), m1s)
...
def hyper_prior_turn_on_pow_fix_1p7(dataset, alpha, mmin, delta_m):
    parameters = dict(alpha=alpha, mmin=mmin, delta_m=delta_m)
    pow_norm_fix = norm_ppow_fix(parameters)
    probability_fix = turn_on_pow_fix(dataset['mu'], parameters, pow_norm_fix)
    return probability_fix

hp_priors_turn_on_pow_fix_1p7 = dict(alpha=Uniform(-5, 25, 'alpha', '$\\alpha$'),
                 mmin=Uniform(0.9, 1.5, 'mmin', '$mmin$'),
                delta_m=Uniform(0.01, 1, 'delta', '$\\delta$') )
#end turn_on_pow fixed m_max 1.7

#begin turn_on_pow with fixed m_max 5
fix_max_m_5p=5
def window_fix(masses, mmin,  delta_m):
    mmaxs_fix=fix_max_m_5p
    mmax=mmaxs_fix
...
def norm_ppow_fix(parameters):
    """normalise ppow, requires m1s, an array of m values, and dm, the spacing of
    that array"""
    m1s = np.linspace(0.9, fix_max_m_5p, 500)
    return np.trapz(ppow_fix(m1s, parameters), m1s)
...
def hyper_prior_turn_on_pow_fix_5p(dataset, alpha, mmin, delta_m):
    parameters = dict(alpha=alpha, mmin=mmin, delta_m=delta_m)
    pow_norm_fix = norm_ppow_fix(parameters)
    probability_fix = turn_on_pow_fix(dataset['mu'], parameters, pow_norm_fix)
    return probability_fix

hp_priors_turn_on_pow_fix_5p = dict(alpha=Uniform(-5, 25, 'alpha', '$\\alpha$'),
                 mmin=Uniform(0.9, 1.5, 'mmin', '$mmin$'),
                delta_m=Uniform(0.01, 1, 'delta', '$\\delta$') )
#end turn_on_pow fixed m_max 5

########test different fixed maximum mass affact the evidence of 2G_fixed_max model
#2G fixed max 5 solar mass
mup_5p=5
def hyper_prior_2G_fixed_max_5p(dataset, mu1, sigma1,mu2,sigma2,alpha,mlo):
    mup=mup_5p
    normalisingTerm1 = 0.5 * ( erf((mu1-mlo)/(np.sqrt(2) * sigma1)) -  erf((mu1-mup)/(np.sqrt(2) * sigma1)) )
    normalisingTerm2 = 0.5 * ( erf((mu2-mlo)/(np.sqrt(2) * sigma2)) -  erf((mu2-mup)/(np.sqrt(2) * sigma2)) )
    return ( (mu2 < mup ) & (mu1 > mlo) & (mu1 < mu2)  & ( dataset['mu'] >= mlo) & (dataset['mu'] <= mup)) *\
        ( (( alpha*(np.exp(- (dataset['mu'] - mu1)**2 / (2 * sigma1**2)) /(2 * np.pi * sigma1**2)**0.5)) /normalisingTerm1) +\
        (1-alpha)*( ((np.exp(- (dataset['mu'] - mu2)**2 / (2 * sigma2**2)) /(2 * np.pi * sigma2**2)**0.5) ) / normalisingTerm2) )
hp_priors_2G_fixed_max_5p = dict(mu1=Uniform(0.9, 2.9, 'mu1', '$\mu_1$'),
                 sigma1=Uniform(0.01, 2, 'sigma1', '$\sigma_1$'),
                mu2=Uniform(0.9, 2.9, 'mu2', '$\mu_2$'),
                sigma2=Uniform(0.01, 2, 'sigma2', '$\sigma_2$'),
                alpha=Uniform(0.01, 1, 'alpha', '$\\alpha$'),
                mlo=Uniform(0.9, 1.5, 'mlo',r'$\rm{m^l}$') )
#end 2G fixed max 5 solar mass

#2G fixed max 2.3 solar mass
mup_2p3=2.3
def hyper_prior_2G_fixed_max_2p3(dataset, mu1, sigma1,mu2,sigma2,alpha,mlo):
    mup=mup_2p3
    normalisingTerm1 = 0.5 * ( erf((mu1-mlo)/(np.sqrt(2) * sigma1)) -  erf((mu1-mup)/(np.sqrt(2) * sigma1)) )
    normalisingTerm2 = 0.5 * ( erf((mu2-mlo)/(np.sqrt(2) * sigma2)) -  erf((mu2-mup)/(np.sqrt(2) * sigma2)) )
    return ( (mu2 < mup ) & (mu1 > mlo) & (mu1 < mu2)  & ( dataset['mu'] >= mlo) & (dataset['mu'] <= mup)) *\
        ( (( alpha*(np.exp(- (dataset['mu'] - mu1)**2 / (2 * sigma1**2)) /(2 * np.pi * sigma1**2)**0.5)) /normalisingTerm1) +\
        (1-alpha)*( ((np.exp(- (dataset['mu'] - mu2)**2 / (2 * sigma2**2)) /(2 * np.pi * sigma2**2)**0.5) ) / normalisingTerm2) )
hp_priors_2G_fixed_max_2p3 = dict(mu1=Uniform(0.9, 2.9, 'mu1', '$\mu_1$'),
                 sigma1=Uniform(0.01, 2, 'sigma1', '$\sigma_1$'),
                mu2=Uniform(0.9, 2.9, 'mu2', '$\mu_2$'),
                sigma2=Uniform(0.01, 2, 'sigma2', '$\sigma_2$'),
                alpha=Uniform(0.01, 1, 'alpha', '$\\alpha$'),
                mlo=Uniform(0.9, 1.5, 'mlo',r'$\rm{m^l}$') )
#end 2G fixed max 2.3 solar mass

#2G fixed max 1.7 solar mass
mup_1p7=1.7
def hyper_prior_2G_fixed_max_1p7(dataset, mu1, sigma1,mu2,sigma2,alpha,mlo):
    mup=mup_1p7
    normalisingTerm1 = 0.5 * ( erf((mu1-mlo)/(np.sqrt(2) * sigma1)) -  erf((mu1-mup)/(np.sqrt(2) * sigma1)) )
    normalisingTerm2 = 0.5 * ( erf((mu2-mlo)/(np.sqrt(2) * sigma2)) -  erf((mu2-mup)/(np.sqrt(2) * sigma2)) )
    return ( (mu2 < mup ) & (mu1 > mlo) & (mu1 < mu2)  & ( dataset['mu'] >= mlo) & (dataset['mu'] <= mup)) *\
        ( (( alpha*(np.exp(- (dataset['mu'] - mu1)**2 / (2 * sigma1**2)) /(2 * np.pi * sigma1**2)**0.5)) /normalisingTerm1) +\
        (1-alpha)*( ((np.exp(- (dataset['mu'] - mu2)**2 / (2 * sigma2**2)) /(2 * np.pi * sigma2**2)**0.5) ) / normalisingTerm2) )
hp_priors_2G_fixed_max_1p7 = dict(mu1=Uniform(0.9, 2.9, 'mu1', '$\mu_1$'),
                 sigma1=Uniform(0.01, 2, 'sigma1', '$\sigma_1$'),
                mu2=Uniform(0.9, 2.9, 'mu2', '$\mu_2$'),
                sigma2=Uniform(0.01, 2, 'sigma2', '$\sigma_2$'),
                alpha=Uniform(0.01, 1, 'alpha', '$\\alpha$'),
                mlo=Uniform(0.9, 1.5, 'mlo',r'$\rm{m^l}$') )
#end 2G fixed max 1.7 solar mass
